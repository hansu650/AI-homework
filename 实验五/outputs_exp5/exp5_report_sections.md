...
程序整体流程是：先读取 IMDB 数据集，再清洗影评文本、分词、构建词表，把每条影评 padding/truncate 到 `200`，之后使用 PyTorch 搭建 LSTM 情感分类模型，训练时用验证集做 EarlyStopping，最后在测试集上计算指标并保存图像。
...
![图2 标签分布](exp5_label_distribution.png)

## 图3 文本长度分布截图

对应 `exp5_review_length_distribution.png`。红色虚线表示 `maxlen=200`，超过 200 个 token 的评论会被截断，不足 200 的评论会在后面 padding。

![图3 文本长度分布](exp5_review_length_distribution.png)
...
终端 `Training Logs` 记录了每一轮的 `train_loss`、`val_loss`、`train_acc`、`val_acc`。本次最佳验证集损失出现在第 6 轮，最佳 `val_loss=0.3007`。

## 图7 训练损失曲线和准确率曲线

下面两张图分别展示训练集和验证集的 loss、accuracy 变化。可以通过曲线观察模型是否正常收敛，以及验证集是否开始波动或出现过拟合迹象。

![图7-1 Loss 曲线](exp5_training_loss_curve.png)

![图7-2 Accuracy 曲线](exp5_accuracy_curve.png)

## 图8 测试集混淆矩阵

混淆矩阵展示了负类和正类各自被正确识别、错误识别的数量，能够看出模型在哪一类上更容易出错。

![图8 混淆矩阵](exp5_confusion_matrix.png)

## 图9 测试指标柱状图

测试集真实运行结果如下：Accuracy=0.8748，Precision=0.8635，Recall=0.8904，F1=0.8767。

![图9 测试指标](exp5_metrics_bar.png)

## 图10 自定义影评预测结果

对应 `exp5_prediction_examples.txt` 和终端 `Sample Predictions`。短句预测结果如下：

- `This movie is fantastic!`：预测为 **positive**，正类概率为 **0.9578**。
- `A waste of time.`：预测为 **negative**，正类概率为 **0.0082**。
- `The story is boring and too long.`：预测为 **negative**，正类概率为 **0.0202**。
- `I really like the actors and the ending is touching.`：预测为 **positive**，正类概率为 **0.8580**。
- `The film starts slowly, but the ending is powerful and very emotional.`：预测为 **positive**，正类概率为 **0.9341**。
- `I expected much more from this movie, but it was messy, dull and too long.`：预测为 **negative**，正类概率为 **0.0130**。
